...
def build_executor_system() -> str:
    return (
        ...
        "referenced file too.\n"
        "9. NOT FOUND = CLARIFICATION: If the task asks for a specific "
        "item (an article from a specific date, a file with a specific "
        "name, a record matching a specific criterion) and you cannot "
        "find an exact match, report OUTCOME_NONE_CLARIFICATION — not "
        "OUTCOME_OK with alternatives or closest matches. Providing "
        "'the closest captures are...' when the exact item was not found "
        "is not an acceptable OK answer.\n"
        "</execution-process>\n"
        "\n"
        f"<outcome-codes>\n{OUTCOME_CODES_DOC}\n</outcome-codes>"
    )
